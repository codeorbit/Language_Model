'''
	...
	This module is the main retrieval for the language model


	1.) len(third_word)<3
		beacuse spelling errors (related to keywords) occur happen after 3 letter of the word(most probably) 
...
# CLOSE_VALUES is for keyboard errors 
CLOSE_VALUES = {
...
def relGivenToPrev(given_word,prev_word):
...
def secondWord(first_word,second_word):
	global REL,CACHE
	probable_words={}
	...
	first_word_count = givenWordCount(first_word)
	

	if len(second_word)<3:
		
		most_probable_words = relGivenToPrev(second_word,first_word)
...
# second_word("google","akhi")


def thirdWord(first_word,second_word,third_word):
	...
	second_word_count = givenWordCount(second_word)

	first_word_count = givenWordCount(first_word) 
		
	rel_first_third = relGivenToPrev(third_word,first_word)

	rel_second_third = relGivenToPrev(third_word,second_word)

	rel_second_first = relGivenToPrev(second_word,first_word)
...
def fourthWord(first_word,second_word,third_word,fourth_word):
	global REL
	probable_words={}
	...
	if len(relGivenToPrev(fourth_word,third_word)):
		REL=True
	else:
		REL=False
